...
const TutorRegistration = () => {
  ...
  const [newLink, setNewLink] = useState({ platform: "", url: "", customPlatform: "" });
  ...
  const handleImageUpload = async () => {
    ...
    const storageRef = ref(storage, `tutors/${Date.now()}_${profileImage.name}`);
  ...
  const addSocialLink = () => {
    ...
    if (selectedPlatform?.pattern && !selectedPlatform.pattern.test(newLink.url)) {
      setMessage(`Please enter a valid ${selectedPlatform.name} URL`);
      return;
    }

    const platformName = newLink.platform === 'other' ? newLink.customPlatform : newLink.platform;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const formData = {
        ...
        profileImageUrl,
        socialLinks,
        specializations: userType === "tutor" ? specializations : undefined,
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="px-8 py-6 bg-gradient-to-r from-blue-600 to-blue-500">
          <h2 className="text-3xl font-bold text-white text-center">
            {userType === "company" ? "Company Registration" : "Become a Tutor"}
          </h2>
          <p className="text-center text-blue-100 mt-2">
            {userType === "company" ? "Join our platform as an employer" : "Join our community of expert educators"}
          </p>
        </div>
                ...
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  ...
                  <div className="mt-1 flex items-center space-x-4">
                    ...
                    <span className="text-sm text-gray-500">
                      {profileImage ? profileImage.name : "Recommended size: 500x500px"}
                    </span>
                  </div>
                </label>
              </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <button
                  ...
                  className="px-6 py-3 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition-colors flex items-center justify-center gap-2 md:col-span-3"
                >
                  <FiPlus className="h-5 w-5" />
                  Add Social Profile
                </button>
                ...
                {socialLinks.length > 0 ? (
                  socialLinks.map((link, index) => {
                    const platform = SOCIAL_PLATFORMS.find(p => p.value === link.platform) || 
                                    { name: link.platformName, icon: <FaGlobe className="text-gray-600" /> };
                  ...
                  <p className="text-gray-500 text-sm">No social profiles added yet</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="sticky bottom-0 bg-white pt-6 border-t">
            ...
            >
              {loading ? (
                ...
                `Complete ${userType === "company" ? "Company" : "Tutor"} Registration`
              )}
            </button>
          </div>
        </form>
